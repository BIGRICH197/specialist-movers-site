...
export function HamiltonPageLink({ serviceSlug, variant = "inline" }: Props) {
  ...
  const linkClass =
    variant === "hero"
      ? "inline-flex rounded-full border border-brand-yellow/40 bg-brand-yellow/10 px-3 py-1.5 text-xs font-semibold text-brand-yellow transition hover:bg-brand-yellow/20"
      : "mt-4 inline-flex rounded-full border border-brand-purple/20 bg-white px-4 py-2 text-sm font-semibold text-brand-purple shadow-sm transition hover:border-brand-purple/40";
