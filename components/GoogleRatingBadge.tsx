...
type Props = {
  className?: string;
  style?: CSSProperties;
  /** Square tile for desktop hero gap; compact bar for mobile; overlay for photo corner */
  variant?: "tile" | "compact" | "overlay";
};
  ...
  if (variant === "overlay") {
    return (
      <Link
        ...
        rel="noopener noreferrer"
        className={cn(
          "group flex aspect-square w-[8.75rem] shrink-0 flex-col items-center justify-center gap-1 rounded-xl border-2 border-white/30 bg-white/95 p-2.5 text-center text-brand-purple shadow-[0_12px_32px_-10px_rgba(0,0,0,0.45)] backdrop-blur-sm transition-[border-color,box-shadow] hover:border-brand-yellow/50 hover:shadow-[0_14px_36px_-8px_rgba(151,57,176,0.4)] sm:w-[9.25rem]",
          className,
        )}
        style={style}
        aria-label={label}
      >
        <Image
          ...
          width={64}
          height={20}
          className="mx-auto block h-3.5 w-auto"
          aria-hidden
        />
        <p className="w-full text-center font-heading text-2xl font-bold leading-none text-brand-purple">
          {googleRating}
        </p>
        ...
        <p className="w-full px-0.5 text-center text-[9px] font-semibold leading-tight tracking-wide text-brand-purple/80">
          {reviewLine(googleReviewCount)}
        </p>
      </Link>
    );
