...
- **Review sync** , automate Google reviews refresh (Places API or periodic scrape of `/reviews/`).
- **Hamilton zone distances** , wire Google Maps cache for Zone A/B/C (website uses suburb keywords for now).
- **Service terms** , full booking T&Cs (deposits, damage, weather) when solicitor supplies wording; privacy policy is live on `/policies`.
